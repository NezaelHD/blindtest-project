            ...
            <form action="/profil" method="post" class="Large__Form" autocomplete="off">
                <div class="auth__form_body">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="text" name="name" id="name" value="<?= $data['user']['name']; ?>" required class="Large__Input">
                    </div>
                    <div class="form-group mb-4">
                        <label class="text-uppercase small custom-label">E-Mail</label>
                        <input type="email" name="email" id="email" value="<?= $data['user']['email']; ?>" required class="Large__Input">
                    </div>
                    <div class="form-group mb-4">
                        <label class="text-uppercase small custom-label">Mot de Passe</label>
                        <input type="password" name="password" id="password" value="<?= $data['user']['password']; ?>" required class="Large__Input">
                    </div>
                    ...
                    <?php foreach ($data['blindtests'] as $blindtest): ?>
                        <?php if ($blindtest->getAuthor() === $data['user']['id']): ?>
                            <tr id="blindtest-<?= $blindtest->getId() ?>">
                                ...
                                <td>
                                    <button class="btn-danger btn-sm blind-test-modal" data-type="delete" data-param="<?= $blindtest->getId() ?>">Supprimer</button>
                                    <button class="btn-primary btn-sm blind-test-modal" data-type="edit" data-param="<?= $blindtest->getId() ?>">Éditer</button>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php endforeach; ?>
